...
void LidarLite::setupLidar(char _lidarliteAddress, 
                            PinName _enablePin,
                            std::string name) {
...
bool LidarLite::reset() {
    ...
    /* First, disable all other LidarLites except the LidarLite to be reset.
     *  This means that the LidarLite to be reset will have the default 
     *  address, 0x62.
     */
    LidarLiteController::disableAll();
    enable();
    
    // Next, attempt to reset the LidarLite's I2C address and config values
    i2cSuccessful &= changeI2CAddress(LIDARLITE_ADDR_DEFAULT, lidarliteAddr);
    i2cSuccessful &= setLidarSettings();
    
    // Finally, re-enable all of the lidars
    LidarLiteController::enableAll();
    
    /* If a read/write failed, the LidarLite is unreachable. Return false. 
     *  Otherwise, the LidarLite was successfully reset. Return true.
     */
    if (i2cSuccessful) {
        return true;
    } else {
...
int LidarLite::getDistance() {
     ...
     */
    if(measurementsNoBiasCorrection < 99) {
        dataArr[0] = 0x03;
...
bool LidarLite::write(char registerAddr, char* dataArr, int numBytes, 
